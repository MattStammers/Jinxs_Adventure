...
class MyGame(arcade.Window):
    ...
    def add_coins(self):
        ...
        for i in range(COIN_COUNT_INCREMENT):
            ...
            coin = arcade.Sprite(":resources:images/animated_characters/male_person/malePerson_idle.png", SPRITE_SCALING_COIN)
    ...
    def update(self, delta_time):
        ...
        for player in self.player_list:
            if player.center_x < 0 and self.player.change_x < 0:
                player.change_x *= -1
            if player.center_y < 0 and self.player.change_y < 0:
                player.change_y *= -1

            if player.center_x > SCREEN_WIDTH and self.player.change_x > 0:
                player.change_x *= -1
            if player.center_y > SCREEN_HEIGHT and self.player.change_y > 0:
                player.change_y *= -1

            coin_hit_list = arcade.check_for_collision_with_list(player, self.coin_list)
